...
public class BoardService {
	...
	public BoardDTO save(BoardDTO boardDTO) throws IOException {
		...
		boardDTO.setBoardContents(boardDTO.getBoardContents().replace("\n", "<br>"));
		...
		if (boardDTO.getBoardFile().get(0).isEmpty()) {	// 저장 시 첨부 된 파일 유무 조회
			...
			for (MultipartFile boardFile : boardDTO.getBoardFile()) {
				...
				String savePath = "/springboot-board/src/main/resources/upload_files/" + storedFileName;
				// 실질적으로 파일이 저장되는 코드
	...
	// 게시글 수정
	public void update(BoardDTO boardDTO, Long id) throws IOException {
		boardDTO.setBoardContents(boardDTO.getBoardContents().replace("\n", "<br>"));
	...
	public void delete(Long id) {
		...
		boardRepository.deleteFile(id);
		boardRepository.deleteComment(id);
